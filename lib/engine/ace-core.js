var AceCore = 'undefined' == typeof exports ? AceCore || {} : exports;
void function(exports){
	 ...
	 */
		logger = {
			log: function(message){
				'object' == typeof console && console.log(message);
			}
		};
	 ...
	 * @param {Function} callback 事件处理完毕回调
	 */
	function fire(event, data, callback){
		if (!active) { // 程序未启动，先做缓存
			eventCaches.unshift([event, data, callback]);
			return;
		}
		
		var listener = listeners[event];
		if (!listener) {
			callback && callback(data, event, 0);
			return;
		}
		var i = listener.length;
		while (i--) {
			//try {
				listener[i](data, event, i);
			//} catch(ex) {
				//logger.log([event, ex.message].join(" : "));
			//}
		}
		callback && callback(data, event, listener.length);
	}
